...
const MovieC = () => {
    ...
    const {nombre, sinopsis, productor, director, clasificacion, tipo} = inputs;
    ...
    const onSubmit = async(e) => {
        e.preventDefault();
        if (nombre !== '' && sinopsis !== '' && productor !== '' && director !== '' && clasificacion !== '' && tipo !== '' && image !== ''){
            ...
            setLoading(true);
            try {
                const response = await axios.post('http://localhost:4000/api/peliculas', formData, {
                  headers: {
                    'Content-Type': 'multipart/form-data',
                  },
                });
                // const decodedToken = jwt.verify(token, 'token'); // Reemplaza 'tu_secreto' con tu clave secreta
                // const userId = decodedToken.id;
          
                setMensaje(response.data.mensaje);
                setInputs({ nombre: "", sinopsis: "", productor: "", director: "", clasificacion: "", tipo: "" });
                ...
                setMensaje('Hubo un error en la carga de datos');
                setTimeout(() => {
                  setMensaje("");
                  setLoading(false);
                }, 1500);
    ...
    <div className={styles.formContainer}>
        ...
        <h2>De Registro de nueva pelicula!</h2>
        <form onSubmit={(e) => onSubmit(e)}>
            ...
            <button type='submit'>
                {loading ? "Cargando..." : "Registrar pelicula"}
            </button>
        </form>
